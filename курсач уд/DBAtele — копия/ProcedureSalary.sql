...
SELECT @datevar;
insert into [dbo].Бухгалтерия.[Зарплата] ([dbo].Бухгалтерия.[ID сотрудника],[dbo].Бухгалтерия.[Дата зарплаты], [dbo].Бухгалтерия.Зарплата,[dbo].Бухгалтерия.Премия) values 
(@IDсотр,@datevar,dbo.FunctionSalary(@Оклад,@Стаж,1),dbo.FunctionSalary(@Оклад,@Стаж,0))
fetch next from @CursPerem into @IDсотр,@Фамилия,@Имя,@Отчество,@Должность,@Приемка,@ОбщийСтаж,@Стаж,@Доступность,@email,@pass,@Оклад
END
close @CursPerem
